...
class RFMInsightGenerator:
    ...
    def generate_clv_insights(
        ...
        if feature_importances and len(feature_importances) > 0:
            top_drivers = list(feature_importances.items())[:5]
            ...
            # Fallback: empty list instead of missing key
            insights["top_drivers"] = []
            if self.verbose:
                print("[RFMInsightGenerator] Warning: No feature importances available for CLV")
